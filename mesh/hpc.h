...
typedef struct mesh_data  /* mesh */
{
    ...
    index *elem ;     /* elements ([e1,e2,e3,m1,m2,m3,t1], ... ), ei := Knotenindex, mi := Kantenindex */
    index *edge2no ;  /* vector of length 2*nedges with start/endpoint for each edge (start1,end1,start2,end2...)*/
    index *bdry ;     /* bdry ([e1,e2,m1,t1], [e3,e4,m2,t2], ...) , t1=0 if fixed*/
    index *fixed ;    /* bdry ([e1,e2,m1,t1], [e3,e4,m2,t2], ...) FALSCH */
                      /* fixed ist Vektor (length nfixed), wo erst Indizes von fixed Knoten, dann von fixed Kanten kommen. Kanten nicht direkt Index, sondern Index+ncoords */
} mesh ;
